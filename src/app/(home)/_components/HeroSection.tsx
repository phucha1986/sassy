export default function HeroSection() {
    return (
        <div className="py-28 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
            <h1 className="text-6xl font-extrabold text-gray-900">Welcome to Micro-SaaS Creator</h1>
            <p className="mt-4 text-lg text-gray-600">
                Empower your vision with tools to build Micro-SaaS solutions effortlessly.
            </p>
            <a href="/dashboard">
                <button className="mt-6 bg-indigo-600 text-white py-3 px-6 rounded-lg hover:bg-indigo-700">Get Started</button>
            </a>
        </div>
    );
}
